...
var initParticles = function() {
    ...
    var createParticlesShader = function() {
        ...
        var fragment = [
            ...
            "      //wind = 3.0;",
            "      float speed = time;",
            "      float t = 1.0*(FragTexCoord0.y*FragTexCoord0.x*5.0*3.1415)*0.6;",
            "      vec3 pos = getEqualizerParticleSpawn(time, equalizerLevel);",
            "      //targetVec += pos;",
            "      if (false && material > 0.95) {",
            "      vec3 vel = currentPosition - prevPosition;",
            "      vec3 dir = cross(vel,vec3(0.0,0.0,1.0));",
            "      dir *= cos(seed*2.0*3.1415*FragTexCoord0.x+FragTexCoord0.y);",
            "      //vec3 vec = currentPosition-(prevPosition+dir);",
            "      //vec3 dif = vec3(cos(FragTexCoord0.x+FragTexCoord0.y+seed), sin(seed*2.0*3.1415+FragTexCoord0.y), 0.0);",
            "      targetVec += normalize(dir)*0.1*(FragTexCoord0.x + FragTexCoord0.y)*(1.0 + seed);",
            "      }",
            "   }",
            "",
        ...
        update: function(node, nv) {
            ...
            if (this.nbUpdate == 0) {
                ...
                if (false) {
                    ...
                    uniformEqualizerLevel.get()[0] = 0.2 + 0.8 * (timeObjects.Text1.value + timeObjects.Text2.value + timeObjects.Text3.value + timeObjects.Text3.value + timeObjects.FRQMusicSnare.value); uniformEqualizerLevel.dirty();
//                    rotationX.set([0.1]);
//                    rotationZ.set([0.2]);
//                    weightVelocityField.set([0.5* (0.5 + 0.5*Math.cos(t*0.2))]);
                }
            }
